...
void init_mem(machbstart_t *mbsp)
{
    ...
    retemp = (e820map_t*)info->Mem.E820Map;
    retemnr = (u32_t)info->Mem.E820MapNR;

    if (retemnr == 0)
    {
        kerror("no e820map\n");
    }
    mbsp->mb_e820padr = (u64_t)(retemp);
    mbsp->mb_e820nr = (u64_t)retemnr;
    mbsp->mb_e820sz = (uint_t)(retemnr * (sizeof(e820map_t)));
    mbsp->mb_memsz = get_memsize(retemp, (uint_t)retemnr);
...
void init_chkcpu(machbstart_t *mbsp)
{
    ...
    // CLI_HALT();
    // if (!chk_cpu_avx512fbit())
    // {
    //     kprint("Your CPU is not support avx512f sys is die!");
    //     CLI_HALT();
    // }
    // kerror("ok");
    return;
...
void init_krlinitstack(machbstart_t *mbsp)
{
    ...
    if (1 > move_krlimg(mbsp, (u64_t)(IKSTACK_PHYADR - IKSTACK_SIZE), IKSTACK_SIZE))
    {
...
void init_meme820(machbstart_t *mbsp)
{
    ...
    e820map_t *demp =(e820map_t*)LoaderAllocMemroy(esz);//=(e820map_t *)((uint_t)(mbsp->mb_nextwtpadr));
    if(NULL == demp)
    {
        Debug();
    ...
    if(1 > move_krlimg(mbsp, (u64_t)((uint_t)demp), esz))
    {
        Debug();
    ...
    mbsp->mb_e820sz = esz;
    // mbsp->mb_nextwtpadr = P4K_ALIGN((uint_t)(demp) + (uint_t)(senr * (sizeof(e820map_t))));
...
uint_t LoaderAllocMemroy(uint_t size)
{
    machbstart_t* mbsp;
    uint_t start, end;
    mbsp = GetMachbstart();
