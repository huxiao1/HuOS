...
{
    if(MemoryIsFree((uint_t)initp, sizeof(machbstart_t)) == FALSE)
    {
        return;
    }
    if(MemoryIsFree(IMGKRNL_PHYADR, 0x8000000) == FALSE)
    {
    ...
    initp->mb_nextwtpadr = IMGKRNL_PHYADR;
    GLmasbp = initp;
    return;
}
